...
//       9크레딧 이하 손실은 대상이 아니다.
//
//       시간창을 두는 이유(2026-07-22 리뷰): 재지급받은 계정이 재체험 크레딧을 다시
//       만료로 날리면 후보 조건에 영원히 재진입
...
//       점유도 건너뛴다.
//
// 지급: re_earlybird 코드(15크레딧·유효 7일, 0025)를 'system' 소스로 상환.
    ...
    for (const p of pageRows) {
      ...
      try {
        ...
        if (userError) {
          ...
          // 넘어 이 재지급의 후보가 된다 — 실제로 2계정이 만료일에 재지급을
