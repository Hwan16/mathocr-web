...
// 30크레딧이고 max_uses 가 null(상한 없음)이라, 실수로 활성화되는 순간
// 마이페이지 코드 입력창과 가입 ?promo= 파라미터로 누구나 30크레딧을 받을 수 있고
...
export async function PATCH(
  ...
  // 상한 없는 30크레딧 코드가 공개돼 버린다(관리자는 막혔다고 착각).
      ...
      return NextResponse.json(
        {
          error:
            ...
            "만료 재지급 cron 이 내부적으로만 사용하며, 공개되면 상한 없이 30크레딧이 지급됩니다.",
        },
        { status: 409 }
      );
    }
