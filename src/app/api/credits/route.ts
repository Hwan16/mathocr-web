...
export async function GET() {
  ...
  // 인증 링크가 만든 세션으로 바로 들어오거나 비밀번호 재설정으로 진입하면
  // 그 경로를 건너뛴다. 이 API는 앱이 변환 전마다, 웹 대시보드가 진입 시마다
  // 호출하므로 누락된 지급을 여기서 회복한다. RPC가 계정당 1회를 보장하며,
  // 이미 지급된 계정은 claimSignupCredits 가 즉시 반환한다.
